...
function contextConstructor (userConfig = {}) {
  ...
  function hideChildren (e) {
    ...
    const visibility = parentComment.getAttribute('data-visibility')

    parentComment.classList.toggle('closed')
    parentComment.setAttribute('data-visibility', visibility === 'hidden' ? 'visible' : 'hidden')
  }
  ...
  function showMoreComments (e) {
    ...
    function showComment (c, count) {
      if (c && count !== 3) {
        if (c instanceof window.Text || getDisplayVal(c) === 'block') {
